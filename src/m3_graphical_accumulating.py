...
def draw_lines(n, point, window):
    for k in range(n):
        distance  = 200/(n - 1)
        leftmost = rg.Point(point.x, point.y)
        rightmostxpoint = leftmost.x + 100
        rightmostypoint = ((leftmost.y - 100) + distance * k)
        rightmost = rg.Point(rightmostxpoint, rightmostypoint)
        line = rg.Line(leftmost, rightmost)
        line.attach_to(window)
    window.render()
    """
